...
async function restartApp(dataDir, win) {
    ...
    try {
        ...
        serverProcess = fork(serverEntry, [], {
            env: { ...process.env, userDir: dataDir, PORT: 1882 },
            silent: false
        });
    ...
    try {
        ...
        const serverReady = await waitForServer('http://localhost:1882', 30000, 1000);
        if (!serverReady) {
            throw new Error('Server did not become ready on http://localhost:1882 within 30 seconds');
        }

        await win.loadURL('http://localhost:1882');
        console.log('UI loaded: http://localhost:1882');
