        ...
        return recentProjects.map(project => ({
            ...
            click: async () => {
                try {
                    ...
                    await dialog.showErrorBox('SCADA Error', `Failed to open project: ${error.message}`);
...
async function createNewProject(parentWin) {
    try {
        const { canceled, filePath } = await dialog.showSaveDialog(parentWin, {
            ...
            buttonLabel: 'Create',
            filters: [{ name: 'SCADA Project', extensions: [''] }],
        ...
        await dialog.showErrorBox('SCADA Error', `Failed to create project: ${error.message}`);
        return null;
    }
}
...
async function openProject(parentWin) {
    try {
        ...
        await dialog.showErrorBox('SCADA Error', `Invalid project: The selected folder is not a valid FUXA project. Please select a folder containing a 'data/_appdata' directory.`);
        return null;
    }
}
            ...
            submenu: [
                {
                    ...
                    click: async () => {
                        try {
                            ...
                            await dialog.showErrorBox('SCADA Error', `New project failed: ${error.message}`);
                        }
                    }
                },
                {
                    ...
                    submenu: [
                        {
                            ...
                            click: async () => {
                                try {
                                    ...
                                    await dialog.showErrorBox('SCADA Error', `Open project failed: ${error.message}`);
                                }
                            }
                        },
                        { type: 'separator' },
...
// Wait for SCADA HTTP server to become reachable
...
// Restart SCADA server with new dataDir
async function restartApp(dataDir, win) {
    ...
    try {
        ...
        await dialog.showErrorBox('SCADA Error', `Failed to start server: ${error.message}`);
        return;
    }

    // Reload UI
    try {
        // Update loading screen to show SCADA is loading
        const htmlPath = getHtmlPath('loading.html');
        ...
        await dialog.showErrorBox('SCADA Error', `Failed to load UI: ${error.message}`);
    }
}

// Start SCADA server and load UI
