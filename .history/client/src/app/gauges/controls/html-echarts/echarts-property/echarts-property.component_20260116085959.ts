...
export class EchartsPropertyComponent implements OnInit, OnDestroy {
    ...
    onVariableChange(deviceTagValue: any, index: number) {
        if (deviceTagValue && deviceTagValue.variableId) {
            this.variableIds[index] = deviceTagValue.variableId;
            this.property.variableIds = [...this.variableIds];
            this.onPropertyChanged();
        }
    }

    onAddVariable() {
        this.variableIds.push('');
        this.property.variableIds = [...this.variableIds];
    }

    onRemoveVariable(index: number) {
        this.variableIds.splice(index, 1);
        this.property.variableIds = [...this.variableIds];
        this.onPropertyChanged();
    }
