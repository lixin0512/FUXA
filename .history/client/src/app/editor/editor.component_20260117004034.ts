...
export class EditorComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.libWidgetsService.svgWidgetSelected$.pipe(
            ...
            this.indexedDB.setItem(widgetPath, content).subscribe({
                next: () => {},
                error: (err) => console.error('保存小部件内容失败:', err)
            });
        ...
        // 处理图片格式的小部件
        this.libWidgetsService.imageWidgetSelected$.pipe(
            takeUntil(this.destroy$)
        ).subscribe((widgetPath) => {
            this.imagefile = widgetPath;
            let self = this;
            // 使用 URL 方式添加图片
            if (self.winRef.nativeWindow.svgEditor.setUrlImageToAdd) {
                self.winRef.nativeWindow.svgEditor.setUrlImageToAdd(widgetPath);
            }
            self.setMode('image');
        });
    }
    ...
    private loadHmi(firstTime = false) {
        ...
        if (this.hmi.views?.length <= 0) {
            ...
            this.indexedDB.getItem('@frango.webeditor.currentview').subscribe(oldsel => {
        ...
        if (event.target.files) {
            ...
            if (fileToUpload.type === 'svg') {
                ...
                reader.onloadend = function(e: any) {
                    self.indexedDB.setItem(fileToUpload.name, reader.result.toString()).subscribe({
                        next: () => {},
                        error: (err) => console.error('保存文件内容失败:', err)
                    });
    ...
    onSelectView(view: View, force = true) {
        ...
        this.indexedDB.setItem('@frango.webeditor.currentview', this.currentView.name).subscribe({
            next: () => {},
            error: (err) => console.error('保存当前视图失败:', err)
        });
    ...
    private loadPanelState() {
        this.indexedDB.getItem('@frango.webeditor.panelsState').subscribe(ps => {
            this.panelsState.enabled = true;
            if (ps) {
                this.panelsState = Utils.mergeDeep(this.panelsState, ps);
    ...
    savePanelState() {
        if (this.panelsState.enabled) {
            ...
            this.indexedDB.setItem('@frango.webeditor.panelsState', this.panelsState).subscribe({
                next: () => {},
                error: (err) => console.error('保存面板状态失败:', err)
            });
        }
    }
    //#endregion
    ...
    openEditGauge(settings, callback) {
        ...
        if (!this.currentView || !this.currentView.items) {
            return;
        }
        ...
        if (dlgType === GaugeDialogType.Chart) {
            ...
            this.gaugeDialog.data = {
                settings: tempsettings, devices: Object.values(devices),
                views: hmi.views, dlgType: dlgType, charts: this.projectService.getCharts(),
                names: names
            };
            if (this.sidePanel && !this.sidePanel.opened) {
                this.sidePanel.toggle();
            }
            ...
            dialogRef = this.dialog.open(BagPropertyComponent, {
                position: { top: '30px' },
                data: {
                    settings: tempsettings, devices: Object.values(devices), dlgType: dlgType,
                    names: names
                }
            });
            ...
            dialogRef = this.dialog.open(HtmlSwitchPropertyComponent, {
                ...
                data: {
                    ...
                    inputs: Object.values(this.currentView.items).filter((gs: any) => gs.name && (gs.id.startsWith('HXS_') || gs.id.startsWith('HXI_'))),
                    names: names
                }
            });
            ...
            if (this.reloadGaugeDialog !== undefined) {
                this.reloadGaugeDialog = !this.reloadGaugeDialog;
            }
            return;
        } else {
            ...
            let title = this.getGaugeTitle(tempsettings.type);
            dialogRef = this.dialog.open(GaugePropertyComponent, {
                ...
                data: <GaugePropertyData> {
                    ...
                    inputs: this.currentView?.items ? Object.values(this.currentView.items).filter((gs: any) => gs?.name && gs?.id && (gs.id.startsWith('HXS_') || gs.id.startsWith('HXI_'))) : [],
                    names: names,
                    scripts: (this.projectService?.getScripts && this.projectService.getScripts()) || [],
                    withBitmask: bitmaskSupported,
        ...
        if (dialogRef && callback) {
            dialogRef.afterClosed().subscribe(result => {
                if (result) {
                    ...
                    if (this.currentView && this.saveView) {
                        this.saveView(this.currentView);
                    }
                    if (this.gaugesManager && this.resolver && this.viewContainerRef) {
                        this.gaugesManager.initInEditor(result.settings, this.resolver, this.viewContainerRef, elementWithLanguageText);
                    }
                    if (this.checkSvgElementsMap) {
                        this.checkSvgElementsMap(true);
                    }
                }
            });
    ...
    isSelectedElementToEnableLanguageTextSettings(): any {
        ...
        if (!elementsSelected || !elementsSelected[0]) {
            return null;
        }
    ...
    editBindOfTags(selected: any) {
        ...
        if (elesSelected?.length) {
            ...
            if (eleIdsAndTypes?.length) {
                for (let i = 0; i < eleIdsAndTypes.length; i++) {
                    let gaSrc: GaugeSettings | null = this.searchGaugeSettings ? this.searchGaugeSettings(eleIdsAndTypes[i]) : null;
                    if (gaSrc) {
        ...
        if (this.dialog) {
            ...
            dialogRef.afterClosed().subscribe((result: TagIdRef[]) => {
                if (result?.length && this.currentView) {
    ...
    onGaugeDialogChanged(settings: any) {
        if (!settings || !this.currentView) {
            return;
        }
        if (this.setGaugeSettings) {
    ...
    private getGaugeTitle(type: string): string {
        if (!type || !this.translateService) {
            return '';
        }
    ...
    onAddResource() {
        ...
        dialogRef.afterClosed().subscribe(result => {
            if (result) {
                ...
                const self = this;
                if (this.imagefile) {
                    const fileExt = this.imagefile.split('.').pop()?.toLowerCase();
                    if (fileExt === 'svg') {
                        const imageFile = this.imagefile;
                        fetch(imageFile).then(r => r.text()).then(text => {
                            if (self && self.winRef?.nativeWindow?.svgEditor?.setSvgImageToAdd) {
                                self.winRef.nativeWindow.svgEditor.setSvgImageToAdd(text);
                            }
                            if (self && self.setMode) {
                                self.setMode('svg-image');
                            }
                        });
                    } else if (this.resourcesService && this.imagefile && this.resourcesService.isVideo(this.imagefile)) {
    ...
    onWidgetKiosk() {
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result && this.libWidgetsService) {
    ...
    onFlyEnter() {
        if (this.endActivePointerInteraction) {
    ...
    onHideSelectionToggle(checked: boolean) {
        if (!this.selectedElement || !this.getGaugeSettings) {
            return;
        }
        let gaugeSettings = this.getGaugeSettings(this.selectedElement);
        if (gaugeSettings && this.setGaugeSettings) {
    ...
    onLockSelectionToggle(checked: boolean) {
        ...
        let gaugeSettings = this.getGaugeSettings(this.selectedElement);
        if (gaugeSettings && this.winRef?.nativeWindow?.svgEditor && this.setGaugeSettings) {
